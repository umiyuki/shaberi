...
# do-not-answer-ja-creative-150の結果ファイルを取得
model_result_paths = glob("./data/judgements/*/umiyuki__do-not-answer-ja-creative-150/*.json")

# データセットの名前を定義
eval_dataset_dict = {
    "umiyuki__do-not-answer-ja-creative-150": "Do-Not-Answer-120-ja",
}
...
# CSVに保存
with open("results/do_not_answer_creative_totals.csv", mode="w", encoding="cp932", errors="ignore", newline="") as f:
    eval_res_df.to_csv(f, index=True)

print("Do-Not-Answer-ja-creative-150の集計結果を 'results/do_not_answer_creative_totals.csv' に保存しました。")
